		...
		charname, realmname = strsplit("-", tostring(arg2));
			...
			key, amount = strsplit(" ", arg1);
				...
				if tonumber(amount) > 1000000 then
					...
					NextAmount = strjoin("", AcceptLoserAmount, " ", "a choisi ", BreakUpLargeNumbers(amount), " pièces d'or pour le prochain pari !")
				end
				ChatMsg(NextAmount);
...
function CrossGambling_OnClickSTATS(full)
	...
	if full then
		for k = 0,  #sortlistamount do
			...
			if(sortlistamount[k] < 0) then sortsign = "a perdu"; end
			ChatMsg(string.format(L["%d.  %s %s %d total"], k+1, sortlistname[k], sortsign, math.abs(sortlistamount[k])), chatmethod);
		end
	...
	for i = 0, x1 do
		sortsign = "won";
		if(sortlistamount[i] < 0) then sortsign = "a gagné"; end
		ChatMsg(string.format(L["%d.  %s %s %d total"], i+1, sortlistname[i], sortsign, math.abs(sortlistamount[i])), chatmethod);
	end
