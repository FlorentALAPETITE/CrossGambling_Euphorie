...
function CrossGambling_OnClickSTATS()
	...
	for i = 0, x1 do
		sortsign = "a gagné";
		if(sortlistamount[i] < 0) then sortsign = "a perdu"; end
		SendChatMessage(string.format("%d.  %s %s %d au total", i+1, sortlistname[i], sortsign, math.abs(sortlistamount[i])), chatmethod);
	end
...
function CrossGambling_OnClickROLL()
	...
	if (totalrolls >1) then
		...
		AcceptRolls = "true";
		if (tie == 0) then
			SendChatMessage("== C'est parti pour les rands ! ==",chatmethod,GetDefaultLanguage("player"));
		end
	...
	if (AcceptOnes == "true" and totalrolls <2) then
		SendChatMessage("Pas assez de joueurs ! (Bande de faibles)",chatmethod,GetDefaultLanguage("player"));
	end
end

function CrossGambling_OnClickLASTCALL()
	SendChatMessage("== Dernier appel pour jouer ! ==",chatmethod,GetDefaultLanguage("player"));
	CrossGambling_EditBox:ClearFocus();
	CrossGambling_LASTCALL_Button:Disable();
...
function GetSpecialMessage(lowname, highname)
	...
	if ((lowname == "Sindalar" or lowname == "Syxor" or lowname == "Easydéa") and (highname == "Sindalar" or highname == "Syxor" or highname == "Easydéa")) then
		returnString = "C'est un paladin donc ça va."
	...
	elseif (lowname == "Trollborg" or lowname == "Yheron") then
...
function CrossGambling_Report()
	...
	if (goldowed ~= 0) then
		...
		local string3 = strjoin(" ", "", lowname, "doit donner à", highname, goldowed,"pièces d'or.", specialMessage)
		...
		SendChatMessage("Egalité ! Pas de gagnant sur ce lancé",chatmethod,GetDefaultLanguage("player"));
	end
	CrossGambling_Reset();
	CrossGambling_AcceptOnes_Button:SetText("Open Entry");
	CrossGambling_CHAT_Button:Enable();
...
function CrossGambling_ParseRoll(temp2)
	...
	if junk1 == "obtient" and junk2 == "un" and CrossGambling_Check(player)==1 then
...
function CrossGambling_List()
	for i=1, table.getn(CrossGambling.strings) do
	  	local string3 = strjoin(" ", "", tostring(CrossGambling.strings[i]):gsub("^%l", string.upper),"doit faire son rand.")
		SendChatMessage(string3,chatmethod,GetDefaultLanguage("player"));
	end
end
...
function CrossGambling_ResetCmd()
	SendChatMessage(".:Jeu des PO:. Le jeu a été reset !", chatmethod)	
end
